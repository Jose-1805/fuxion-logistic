$(function () {
    ...
    $('#app-ios').click(function () {
        $('#modal-app-ios').modal('hide');
        window.location.href = "itms-services://?action=download-manifest&url=https://fuxionexpress.fuxionco.net/archivo/app-movil-manifest.plist";
    })
})
...
function cambiarPassword() {
    ...
    $.post(url, params)
        ...
        .fail(function (jqXHR, state, error) {
            abrirAlerta("alertas-cambiar-password", "danger", JSON.parse(jqXHR.responseText), null, null);
            cerrarBlockUiCargando();
        })
}
...
function desbloquearDispositivo() {
    ...
    $.post(url, params)
        ...
        .fail(function (jqXHR, state, error) {
            abrirAlerta("alertas-configuraciones", "danger", JSON.parse(jqXHR.responseText), null, null);
            $('#modal-desbloquear-dispositivo').modal('hide');
            cerrarBlockUiCargando();
        })
}
...
function imagenEmpresario() {
    ...
    $.ajax({
        ...
        error: function (jqXHR, error, state) {
            abrirAlerta("alertas-imagen-empresario","danger",JSON.parse(jqXHR.responseText),null,'body');
            cerrarBlockUiCargando();
        }
    });
}